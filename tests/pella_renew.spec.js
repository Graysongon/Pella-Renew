...
async function handleCuty(page) {
    console.log(`🌐 进入 cuty 页面: ${page.url()}`);

    // ── 点击 submit-button（优先 DOM） ──
    try {
        ...
        console.log('⚠️ DOM 点击失败，尝试 xdotool');

        const rect = await page.locator('#submit-button').boundingBox().catch(() => null);
        if (rect) {
            const { winX, winY, toolbar } = await getWindowOffset(page);
            xdotoolClick(rect.x + winX + rect.width / 2, rect.y + winY + toolbar + rect.height / 2);
        }
    }

    await sleep(3000);

    // ── CF Turnstile（I am not a robot） ──
    const hasCF = await page.locator('iframe[src*="turnstile"]').count();
    if (hasCF) {
        console.log('🛡️ cuty 检测到 CF，开始验证...');
        const ok = await solveTurnstile(page);
        if (!ok) throw new Error('❌ cuty CF 验证失败');
    }

    // ── 等倒计时 ──
    console.log('⏳ 等待 cuty 倒计时...');
    for (let i = 0; i < 60; i++) {
        await sleep(1000);
    ...
    try {
        ...
        console.log('⚠️ GO 点击失败，尝试 JS');
        await page.evaluate(() => document.querySelector('#go')?.click());
    }

    await sleep(5000);
    console.log(`📄 cuty 跳转后: ${page.url()}`);
}
